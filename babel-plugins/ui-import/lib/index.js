...
class Plugin {
  constructor(libraryName, // 需要使用按需加载的包名
  ...
  importMethod(methodName, file, pluginState) {
    if (!pluginState.selectedMethods[methodName]) {
      ...
      const transformedMethodName = transCamel(methodName, ""); // 兼容 windows 路径
      // path.join('antd/lib/button') == 'antd/lib/button'

      const path = winPath(join(this.libraryName, libraryDirectory, transformedMethodName)); // 生成 import 语句
  ...
  ImportDeclaration(path, state) {
    ...
    if (!node) return; // 代码里 import 的包名

    const {
      value
    } = node.source; // 配在插件 options 的包名

    const {
      libraryName
    } = this; // babel-type 工具函数

    const {
      types
    } = this; // 内部状态

    const pluginState = this.getPluginState(state); // 判断是不是需要使用该插件的包
  ...
  CallExpression(path, state) {
    ...
    const pluginState = this.getPluginState(state); // 如果方法调用者是 Identifier 类型
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (({
  ...
  const Program = {
    enter(path, {
      ...
      // 初始化插件实例
      if (!plugins) {
        plugins = new _plugin__WEBPACK_IMPORTED_MODULE_0__["default"](opts.libraryName, opts.stylePath, types);
      }
