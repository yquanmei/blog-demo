...
export default class Plugin {
  constructor({ libraryName, uiSymbol, types }) {
  ...
  _importMethod(methodName, file, pluginState, objectType) {
    ...
    if (
      ...
      switch (methodName) {
        case "zhCN":
          break;
        default: // MallButton -> mall-button
          ...
          const transformdMethodNameForStyleForAntd = transSymbol(
            transformdMethodNameForStyle,
            this.uiSymbol
          );
          addSideEffect(
            file.path,
            `antd/es/${transformdMethodNameForStyleForAntd}/style`
          );
          const path = winPath(
            join(
              this.libraryName,
