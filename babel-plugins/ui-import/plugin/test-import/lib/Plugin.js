 ...
 * @FilePath: /learn-demo/babel-plugins/ui-import/plugin/test-import/es/Plugin.js
 * @Description:
...
function transSymbol(_str, symbol) {
  const str = _str.replace(`${symbol}-`, "");
...
class Plugin {
  ...
  _importMethod(methodName, file, pluginState, objectType) {
    ...
    if (!objectType && !pluginState.selectedmethods[methodName] && !excludedMethodNames.includes(methodName)) {
      let transformdMethodName = methodName; // MallButton
      ...
      switch (methodName) {
        ...
        default:
          // MallButton -> mall-button
          // transformdMethodName = transCamelFirst(methodName);
          transformdMethodName = transCamel(methodName, "-");
          const transformdMethodNameForStyle = transCamel(methodName, "-");
          const transformdMethodNameForStyleForAntd = transSymbol(transformdMethodNameForStyle, this.uiSymbol);
          (0, _helperModuleImports.addSideEffect)(file.path, `antd/es/${transformdMethodNameForStyleForAntd}/style`);
          const path = winPath((0, _path.join)(this.libraryName, libraryDirectory, transformdMethodNameForStyle));
          (0, _helperModuleImports.addSideEffect)(file.path, `${path}/style`);
          break;
      }

      const path = winPath((0, _path.join)(this.libraryName, libraryDirectory, transformdMethodName));
    ...
    if (objectType === "Icon") {
      ...
      const path = winPath((0, _path.join)(realLibraryName, transformdMethodName));
      pluginState.selectedmethods[methodName] = (0, _helperModuleImports.addDefault)(file.path, path, {
        nameHint: methodName
      });
    }
  ...
  CallExpression(path, state) {
    ...
    node.arguments = node.arguments.map(arg => {
      ...
      if (pluginState.specified[argName] && path.scope.hasBinding(argName) && path.scope.getBinding(argName).path.type === "ImportSpecifier") {
        return this._importMethod(pluginState.specified[argName], file, pluginState);
      }
