...
class Plugin {
  ...
  _importMethod(methodName, file, pluginState, objectType) {
    ...
    if (
      ...
      const path = winPath(
        (0, _path.join)(
          this.libraryName,
          libraryDirectory,
          transformdMethodName
        )
      );
      pluginState.selectedmethods[methodName] = (0,
      _helperModuleImports.addDefault)(file.path, path, {
        nameHint: methodName,
      });
    }
    ...
    if (objectType === "Icon") {
      ...
      let transformdMethodName = methodName;
      const path = winPath(
        (0, _path.join)(realLibraryName, transformdMethodName)
      );
      pluginState.selectedmethods[methodName] = (0,
      _helperModuleImports.addDefault)(file.path, path, {
        nameHint: methodName,
  ...
  MemberExpression(path, state) {
    ...
    if (
      !node ||
      (node && !node.object) ||
      (node && node.object && !node.object.name)
    )
      return;
